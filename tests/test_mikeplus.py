...
from modelskill.network._mikeplus import CONTRACT_COLUMNS, resolve_stations
...
def test_returns_contract_columns(db):
    df = resolve_stations(db, item_names=["item_pressure_1"], quantity="Pressure")

    assert list(df.columns) == CONTRACT_COLUMNS
    assert len(df) == 1


def test_junction_and_tank_are_nodes(db):
    df = resolve_stations(
        db, item_names=["item_pressure_1", "item_pressure_2"], quantity="Pressure"
    )

    assert set(df["kind"]) == {"node"}
    assert set(df["location"]) == {"wNode_1", "Tank_A"}


def test_link_without_chainage_is_a_reach(db):
    df = resolve_stations(db, item_names=["item_flow_1"], quantity="Flow")

    assert df["kind"].tolist() == ["reach"]
    assert df["location"].tolist() == ["Pipe_7"]


def test_link_with_chainage_is_a_node_at_a_breakpoint(tmp_path):
    path = build_db(
    ...
    df = resolve_stations(path, item_names=["item_flow_1"], quantity="Flow")

    assert df["kind"].tolist() == ["node"]
    assert df["location"].tolist() == [("Pipe_7", 24.5)]
...
def test_several_items_at_one_location_all_survive(tmp_path):
    ...
    df = resolve_stations(
        path, item_names=["before_valve", "after_valve"], quantity="Pressure"
    )

    assert len(df) == 2
    assert df["location"].tolist() == ["wNode_1", "wNode_1"]
    assert df["name"].tolist() == ["PT.401", "PT.402"]
...
def test_name_falls_back_to_item_name_when_assetnames_collide(tmp_path):
    ...
    df = resolve_stations(path, item_names=["item_a", "item_b"], quantity="Pressure")

    assert df["name"].tolist() == ["item_a", "item_b"]


def test_quantity_is_inferred_when_unambiguous(db):
    df = resolve_stations(db, item_names=["item_pressure_1", "item_pressure_2"])

    assert df["quantity"].tolist() == ["Pressure", "Pressure"]


def test_ambiguous_quantity_raises_and_lists_options(db):
    with pytest.raises(ValueError, match="cannot be inferred") as excinfo:
        resolve_stations(db, item_names=["item_pressure_1", "item_flow_1"])

    assert "Pressure" in str(excinfo.value)
    assert "Flow" in str(excinfo.value)


def test_kind_narrows_the_pool_used_for_inference(db):
    df = resolve_stations(
        db,
        item_names=["item_pressure_1", "item_pressure_2", "item_flow_1"],
        kind="node",
    )
...
def test_unknown_quantity_raises(db):
    with pytest.raises(ValueError, match="not found"):
        resolve_stations(db, item_names=["item_pressure_1"], quantity="Discharge")


def test_wrong_kind_raises_naming_the_actual_kind(db):
    with pytest.raises(ValueError, match="not 'node'"):
        resolve_stations(db, item_names=["item_flow_1"], quantity="Flow", kind="node")
...
def test_missing_items_raise_and_separate_the_two_causes(db):
    with pytest.raises(ValueError) as excinfo:
        resolve_stations(
            db,
            item_names=["item_pressure_1", "PT.401", "never_heard_of_it"],
            quantity="Pressure",
        )
...
def test_missing_items_can_be_skipped(db):
    df = resolve_stations(
        db,
        item_names=["item_pressure_1", "never_heard_of_it"],
        quantity="Pressure",
        on_missing="skip",
    )

    assert df["item_name"].tolist() == ["item_pressure_1"]
...
def test_source_selects_between_files(tmp_path):
    ...
    df = resolve_stations(
        path, item_names=["item_a"], quantity="Pressure", source="main.dfs0"
    )
...
def test_item_registered_against_several_files_raises_without_source(tmp_path):
    ...
    with pytest.raises(ValueError, match="more than one file"):
        resolve_stations(path, item_names=["item_a"], quantity="Pressure")
...
def test_accepts_an_open_connection(db):
    ...
    try:
        df = resolve_stations(conn, item_names=["item_flow_1"], quantity="Flow")
    finally:
        conn.close()
...
def test_missing_table_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="missing table"):
        resolve_stations(path, item_names=["item_a"])
