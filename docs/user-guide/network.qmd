...
by default, e.g. `networkx`. You can install them alongside `modelskill` using the _network_ extra:
...
You can build a `Network` object by loading it from a supported network result file. Reading these files relies on [mikeio1d](https://github.com/DHI/mikeio1d), so install the `network` dependency group first.
