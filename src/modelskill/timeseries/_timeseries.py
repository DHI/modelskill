...
def _validate_dataset(ds: xr.Dataset) -> xr.Dataset:
    ...
    if gtype not in valid_gtypes:
        raise ValueError(
            f"data attribute 'gtype' must be one of {GeometryType.POINT}, {GeometryType.TRACK}, {GeometryType.VERTICAL}"
        )
    ...
    if ds.attrs["gtype"] not in [
        ...
        raise ValueError(
            f"data attribute 'gtype' must be one of {GeometryType.POINT}, {GeometryType.TRACK}, {GeometryType.VERTICAL}, or {GeometryType.NODE}"
        )
