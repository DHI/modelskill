...
class DfsuModelResult(SpatialField):
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(data, mikeio.Dataset):
            data = data[self.sel_items.all]  # IS THIS A BUG? Sel items not set here
    ...
    def _extract_vertical_column(
        ...
        elemids = self.data.geometry.find_index(x=observation.x, y=observation.y)
        if len(elemids) == 0:
            raise ValueError(
                f"No elements found at observation location ({observation.x}, {observation.y})"
            )
